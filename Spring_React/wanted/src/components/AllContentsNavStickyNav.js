...
const AllContentsNavStickyNav = () => {
  ...
  //스크롤 시 All_Contents_Nav_Sticky_Div_Div 부분 고정시키기.
  const topCategory = useRef(null);
  const handleScroll = () => {
    if (window.scrollY > 0) {
      topCategory.current.style.position = "sticky";
    }
  };
  useEffect(() => {
  ...
  return (
    <div className="All_Contents_Nav_Sticky_Div_Div" ref={topCategory}>
...
export default AllContentsNavStickyNav;
